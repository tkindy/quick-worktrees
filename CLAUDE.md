...
4. `lib/config.ts` - Loads optional `conductor.json` from repo root for setup scripts
5. `lib/iterm.ts` - Opens new iTerm2 window via AppleScript, runs setup script if configured

**Worktree naming:** `{repo-name}-{random-word}` placed in parent directory of current repo

**Optional config:** Repos can have `conductor.json` with `scripts.setup` to run in new worktree window
